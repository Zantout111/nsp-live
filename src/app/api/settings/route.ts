...
import {
  applyHeroSectionThemePlan,
  fetchHeroSectionThemeRow,
  planHeroSectionThemePersist,
...
export async function PUT(request: Request) {
  try {
    ...
    const heroPlan = planHeroSectionThemePersist(body as Record<string, unknown>);
    await applyHeroSectionThemePlan(
      (sql, ...params) => db.$executeRawUnsafe(sql, ...params),
      savedSettingsId,
      heroPlan
    );
