...
function IconYt({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
export function footerSocialHasAny(urls: FooterSocialUrls): boolean {
  return Boolean(
    urls.facebook?.trim() ||
      urls.x?.trim() ||
      urls.telegram?.trim() ||
...
export function FooterSocialLinks({
  ...
  if (urls.x?.trim()) items.push({ href: urls.x.trim(), label: labelX, Icon: IconX });
